...
def run_phase_drag_audit(fits_file='KiDS_DR4_QSO_candidates.fits'):
    ...
    import json
    print("\n-> Exportando dados do Phase Drag para o Motor 3D...")
    try:
        export_data = []
        for i in range(len(z_f)):
            export_data.append({ "z": round(float(z_f[i]), 4), "t_lost": round(float(t_lost[i] / 1e9), 4), "rrt_model": round(float(rrt_drag_model(z_f[i], eta_found) / 1e9), 4) })
        caminho_saida = os.path.join("..", "public", "data", "phasedrag.json")
